...
public class FMClient : MonoBehaviour
{
    ...
    IEnumerator NetworkClientStart()
    {
        ...
        Loom.RunAsync(() =>
        {
            while (!stop)
            {
                ...
                {
                    DebugLog("Client Receiver Timeout");
            ...
            while (_appendQueueReceivedPacket.Count > 0)
            {
                ...
                if (ReceivedData.Length > 2)
                {
                    ...
                    switch (_meta[0])
                    {
                        case 0:
                            //byte data
                            if (Manager != null) Manager.OnReceivedByteDataEvent.Invoke(_data);
                            break;
                        case 1:
                            //string message
                            if (Manager != null) Manager.OnReceivedStringDataEvent.Invoke(Encoding.ASCII.GetString(_data));
                            break;
                        case 2:
                            //network object data
                            if (Manager != null) Manager.Action_SyncNetworkObjectTransform(_data);
                            break;
                    }

                    if (Manager != null) Manager.GetRawReceivedData.Invoke(ReceivedData);
                }
            }
            yield return null;
        }
    ...
    void Sender()
    {
        ...
        {
            DebugLog("client sender timeout");
